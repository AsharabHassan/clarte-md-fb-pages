...
export async function getAcneReviews(): Promise<ReviewsResult> {
  try {
    ...
    // Reviews are an enhancement — degrade to empty on any DB error.
    // Logged as a warning (not console.error) so an unavailable/unconfigured
    // DB doesn't trip the Next.js dev error overlay; it resolves once
    // DATABASE_URL points at a reachable Postgres with a `reviews` table.
    console.warn('getAcneReviews: skipping reviews (DB query failed):', err instanceof Error ? err.message : err);
